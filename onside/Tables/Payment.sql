CREATE TABLE [Payment] (
    ...
    CONSTRAINT [FK_onside.Payment_onside.LicenceInvoice_LicenceInvoiceId] FOREIGN KEY ([LicenceInvoiceId]) REFERENCES [LicenceInvoice] ([Id]),
    CONSTRAINT [FK_onside.Payment_onside.UserCard_UserCardId] FOREIGN KEY ([UserCardId]) REFERENCES [UserCard] ([Id])
);


GO
CREATE NONCLUSTERED INDEX [IX_UserCardId]
    ON [Payment]([UserCardId] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_LicenceInvoiceId]
    ON [Payment]([LicenceInvoiceId] ASC);


GO
CREATE UNIQUE NONCLUSTERED INDEX [UX_Payment_TxnId]
    ON [Payment]([TxnId] ASC);
