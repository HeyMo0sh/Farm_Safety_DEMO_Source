CREATE TABLE [FarmEvent] (
    ...
    CONSTRAINT [FK_onside.FarmEvent_onside.ApplicationUser_CapturingUserId] FOREIGN KEY ([CapturingUserId]) REFERENCES [ApplicationUser] ([Id]),
    CONSTRAINT [FK_onside.FarmEvent_onside.Farm_FarmId] FOREIGN KEY ([FarmId]) REFERENCES [Farm] ([Id])
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [UX_FarmEvent_FarmId_OccurredUtc_CapturingUserId_FarmEventType]
    ON [FarmEvent]([FarmId] ASC, [OccurredUtc] ASC, [CapturingUserId] ASC, [FarmEventType] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_FarmEvent_CapturingUserId_OccurredUtc]
    ON [FarmEvent]([CapturingUserId] ASC, [OccurredUtc] ASC);
